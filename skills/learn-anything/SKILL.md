...
7. Prepare `references/course-brief.md` inside the course directory using the constructor's capable model: learning outcome, starting assumptions, revisable milestones, examples, evidence to look for, and useful hints (maximum 32 KB). The separate browser runner receives this plan on learner turns; select its faster model in the browser without changing the constructor. Review returned capability warnings. Never hide the permissions of the selected execution mode or non-streaming degradation.
...
11. Keep browser interaction primary after launch. Submit a milestone with the completed mentor turn when the learner demonstrates progress; the host persists progress and derives notes. Read `references/mentor-recovery.md` when implementing a mentor adapter, recording milestones, or diagnosing interrupted work.

## Composition rules

- Reuse complete runtime blocks. Configure them; do not regenerate server or browser code from scratch. Lesson content is agent-authored A2UI: compose and change surfaces as the learner's needs evolve, rather than selecting a fixed lesson template.
...
- Scope writable paths to the learning directory, except the host-owned validated teaching-pattern library. Models cannot choose library paths. Treat project source as read-only unless learner explicitly authorizes project edits.
...
Read `references/pedagogy.md` before mentoring, including its software/offline-practice guidance for non-coding lessons. Choose a subject-native activity; code execution is optional. Distinguish observed evidence from learner-reported practice and never claim to see or hear an unobserved performance. For repository or large build goals, inspect only relevant source and turn end goal into working milestones. Teach concepts just before learner needs them. Use progressive hints; do not seize keyboard and complete exercise unless learner asks.

Begin with what the learner wants to understand, create, or perform, expressed in their language and illustrated by one concrete example. Keep profiles, adapters, transports, and sandbox choices backstage unless they are the topic. A learner asking what the current task means is a signal to return to chat and replace the task with a clearer prerequisite.
...
Author the lesson's layout, content, relationships, and sequence dynamically. The catalog defines trusted rendering primitives, not an exhaustive set of teaching activities. Adapt or replace compositions after learner questions and evidence. When a useful original composition generalizes beyond this learner, nominate a clean standalone pattern for automatic local reuse; read `references/teaching-patterns.md` before saving or adapting patterns. Never save a learner transcript, private draft, or execution history as a reusable pattern.

The agent creates and updates the work canvas with actual A2UI v0.9 messages. Read `references/stage-catalog.md` for the component vocabulary and `references/shell-mentor.md` for manual operation. Respond to delivered manual work with `mentor.mjs turn`, preserving its turn identity and revision so text, canvas, continuation, and completion commit together. Standalone `canvas` is compatibility-only and does not complete queued work. Every work turn needs a localized task title, a concrete action, a valid target component, and a compatible structured action type.
...
Use the existing session directory. Preserve transcript, exercises, canvas, progress, and assembly manifest. Reuse the dedicated mentor session and selected model with the same adapter; deliberate adapter migration resets provider-owned identity. Start repaints saved browser state before the persistent mentor reconnects. A Pi mentor reopens its course-owned RPC session; if that native session is missing, bootstrap its context from saved course state. All persistent adapters commit complete turns atomically. Accepted questions survive restart; a response conflicting with newer learner edits remains recoverable through **Retry** or **Dismiss**. Never overwrite drafts or present operational failures as mentor teaching. Submit code only with a result produced by that exact code.
...
Success requires health response, browser assets, real Send-to-mentor round trip, visible dynamic focus changes, working current controls, persistence across refresh, a working rescue path, and no blocking design-QA failure in the tested composition. Verify an execution runner when the lesson includes runnable code; non-coding lessons do not require a code exercise.
